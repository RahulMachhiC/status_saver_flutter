...
final Directory whatsapp =
    Directory('Android/media/com.whatsapp/WhatsApp/Media/.Statuses');

class RecentStatusWAScreen extends StatefulWidget {
  const RecentStatusWAScreen({Key? key}) : super(key: key);

  @override
  State<RecentStatusWAScreen> createState() => _RecentStatusWAScreenState();
}

class _RecentStatusWAScreenState extends State<RecentStatusWAScreen> {
  @override
  void initState() {
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    print("bb");
    var imageList = whatsapp.listSync().map((item) {
      print(item.path);
      return item.path;
    }).where((element) {
      return element.endsWith(".jpg");
    }).toList(growable: false);
    var videoList = whatsapp.listSync().map((item) {
      print(item.path);
      return item.path;
    }).where((element) {
      return element.endsWith(".mp4");
    }).toList(growable: false);
    var alllist = whatsapp.listSync().map((e) {
      return e.path;
    }).where((element) {
